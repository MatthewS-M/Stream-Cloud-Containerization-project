...
## Атрибуты качества
|Сценарии\Атрибуты|Источник|Стимул|Артефакт|Реакция|Количественная мера реакции|
|---|---|---|---|---|---|
|Производительность|Поток сообщений, считываемый консьюмером в Kafka|Система должна обрабатывать потоковые данные с нагрузкой в 5 заказов в минуту без снижения производительности|Для этого был внедрен счетчик времени с использованием модуля time, который начинает отсчет от начала чтения первого батча до истечения 5 минут.|За каждые 5 минут идет проверка на количество считанных и обработанных сообщений за интервал времени. При успешном преодолении порога сообщений за промежуток времени, сообщение обрабатывается и переходит в следующий слой.|Реакция (логирование, уведомляющие об успешном счытывании очереди) отображается сразу после проверки на количество сообщений за промежуток времени.|
|Надежность|Брокер сообщений Kafka|Система обеспечивает идемпотентность обработки сообщений из брокера, что гарантирует, что повторная обработка одного и того же сообщения не приведет к изменению состояния базы данных|В моей проектной работе данный сценарий (надежность) реализован путем использования метода для вставки и обновления данных, если таковые уже записаны – upsert.|Если запись с текущим **Id** уже имеется в БД, то она обновляет свои атрибуты, при этом **ID** остается неизменным.|Сценарий реагирует сразу же после завершения транзакции вставки нового сообщения|
|Масштабируемость|Оркестратор контейнеров Kubernetes|Система способна масштабироваться для обработки увеличенного объема данных без значительного снижения производительности. (Автоматическое масштабирование кластера (**Cluster Autoscaler**) и **Горизонтальное масштабирование** подов  - (**Horizontal Pod Autoscaler**))|Это было измерено путем мониторинга использования ресурсов в облачном провайдере при увеличении нагрузки.|Система автоматически отслеживает нагрузку на узлы и при необходимости изменяет их количество в указанных пределах. Kubernetes динамически изменяет количество подов, запущенных на каждом узле группы.|Непосредственно после выявления нагрузки на систему.|
|Устойчивость к ошибкам|Оркестратор контейнеров Kubernetes|Микросервисы должны быть устойчивы к отказам отдельных компонентов системы. Например, если один из микросервисов или компонентов инфраструктуры (например, брокер сообщений или база данных) становится недоступным, система должна продолжать работать без значительного снижения функциональности.|Распределение нагрузки (Load Balancing): Kubernetes распределяет трафик между подами (контейнерами), работающими на узлах кластера, с помощью сервисов и ингресс-контроллеров. Самовосстановление (Self-healing): Kubernetes автоматически отслеживает состояние всех подов и узлов в кластере.|Если какой-либо под или узел выходит из строя, Kubernetes автоматически перезапускает или перемещает его на другие доступные узлы, чтобы восстановить желаемое состояние.|Все сценарии обеспечения отказоустойчивость начинают запускаться сразу после выявления слабых мест системы|
|Соответствие контрактам|Поток сообщений, записываемый в выходной топик продюмером в Kafka|Структура сообщений, отправляемых в Kafka, должна быть фиксированной, чтобы обработчик смог распарсить сообщение|Была установленная стандартизарованная и унифицированная схема выходного и входного сообщений|Было проверено путем тестирования обработчика на различных входных данных и проверки, что он корректно обрабатывает сообщения в соответствии с ожидаемым форматом|При считываннии сообщения консьюмером и перед записью продюсером в новый топик|
|Безопасность|Брокер сообщений Kafka & БД PostgreSQL|Должны быть предприняты меры безопасности при взаимодействии и хранении данных, хоть они и не являются чувствительными|Для обеспечения безопасности данных в процессе передачи Kafka использует TLS (Transport Layer Security) для шифрования данных между клиентами и брокерами Kafka|Система обеспечивает защиту данных, передаваемых через Kafka и хранящихся в PostgreSQL. Это включает в себя шифрование данных в передаче и хранении, а также использование аутентификации и авторизации для доступа к сервисам.|На протяжении всего workflow (пока сервисы работают и подняты в облачном провайдера Yandex Cloud)|
